      ...
      SELECT CASE (TRIM(write_type))
         CASE('INIT')
            IF (myworkid == master) THEN
               ...
               CALL write_var_hdf5(qid_gid,'DIST_NBIN_RHO',ier,DBLVAR=DBLE(ndistns))
               CALL write_var_hdf5(qid_gid,'DIST_NBIN_THETA',ier,DBLVAR=DBLE(16))
               CALL write_var_hdf5(qid_gid,'DIST_NBIN_PHI',ier,DBLVAR=DBLE(ndist2))
               ...
               CALL write_var_hdf5(qid_gid,'zmin',ier,DBLVAR=zaxis(1))
               CALL write_var_hdf5(qid_gid,'zmax',ier,DBLVAR=zaxis(nz))
               CALL write_var_hdf5(qid_gid,'npsi',ier,INTVAR=16)
               ...
               CALL write_var_hdf5(qid_gid,'ntheta',ier,INTVAR=16)
               CALL write_var_hdf5(qid_gid,'nthetag',ier,INTVAR=16)
               CALL write_var_hdf5(qid_gid,'r0',ier,DBLVAR=req_axis(1))
               CALL write_var_hdf5(qid_gid,'z0',ier,DBLVAR=zeq_axis(1))
               CALL write_var_hdf5(qid_gid,'nrzs',ier,INTVAR=16)
               ALLOCATE(rtemp(16,1,1))
               rtemp = 0;
               CALL write_var_hdf5(qid_gid,'rs',16,ier,DBLVAR=rtemp(:,1,1))
               CALL write_var_hdf5(qid_gid,'zs',16,ier,DBLVAR=rtemp(:,1,1))
               DEALLOCATE(rtemp)
               ALLOCATE(rtemp(nr,nz,1))
               rtemp =0
               CALL write_var_hdf5(qid_gid,'psi_rz',nr,nz,ier,DBLVAR=rtemp(:,:,1))
               DEALLOCATE(rtemp)
               ALLOCATE(rtemp(16,16,1))
               rtemp =0
               CALL write_var_hdf5(qid_gid,'theta_psithetageom',16,16,ier,DBLVAR=rtemp(:,:,1))
               CALL write_var_hdf5(qid_gid,'nu_psitheta',16,16,ier,DBLVAR=rtemp(:,:,1))
               DEALLOCATE(rtemp)
               CALL h5gclose_f(qid_gid, ier)
         ...
         CASE('MARKER')
            ...
            IF (myworkid == master) THEN
               ...
               CALL write_var_hdf5(qid_gid,'DIST_NBIN_PR',ier,DBLVAR=DBLE(ndist4))
               CALL write_var_hdf5(qid_gid,'DIST_NBIN_PPHI',ier,DBLVAR=DBLE(ndist4))
               CALL write_var_hdf5(qid_gid,'DIST_NBIN_PZ',ier,DBLVAR=DBLE(ndist4))
               CALL write_var_hdf5(qid_gid,'DIST_NBIN_PPA',ier,DBLVAR=DBLE(ndist4))
               CALL write_var_hdf5(qid_gid,'DIST_NBIN_PPE',ier,DBLVAR=DBLE(ndist5))
               CALL h5gclose_f(qid_gid, ier)
               CALL h5gclose_f(options_gid, ier)
               ! Close file
               CALL close_hdf5(fid,ier)
